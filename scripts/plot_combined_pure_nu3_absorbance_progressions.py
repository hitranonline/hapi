...
then selects one source per J pair using the ordered `--source-priority` list.
...
- `--source-priority` to choose which database wins when both contain the same J pair
...
from research.combined import DEFAULT_SOURCE_PRIORITY, SOURCE_CHOICES, plot_combined_pure_nu3_absorbance_progressions
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot combined pure-nu3 absorbance progressions with configurable source priority.",
    )
    parser.add_argument(
        "--exomol-input-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=SOURCE_CHOICES,
        help="Ordered source precedence for overlapping J pairs. Default: hitran exomol.",
    )
...
def main() -> int:
    ...
    result = plot_combined_pure_nu3_absorbance_progressions(
        ...
        source_priority=tuple(args.source_priority),
        wn_min=args.wn_min,
        wn_max=args.wn_max,
    ...
    for row in result.rows:
        print(
            ...
            f"hitran={row['hitran_selected_jpair_count']} "
            f"exomol={row['exomol_selected_jpair_count']} "
